...
**Before you drive the CLI, read `.claude/bit-cli.md`** — the shared command contract (find the track, list its bars, read a bar body, set status, roll the track up). Every write goes through `./bin/bit`; never hand-edit `.bit/tasks/*.md`.
...
Find the **track**. The user usually names the work when they trigger this ("implement the geographies track"); resolve it to a track ID with `./bin/bit task list` (tracks are the rows whose ID has no dot — match on title). If they didn't name it and more than one track has unfinished bars, list what you found and ask which one — don't pick for them.

Read the track body for context (`./bin/bit task read <track> --body`), then list its bars in order (`./bin/bit task list --parent <track>`). The next bar is the **first one whose status is not `done`** — the status field is the resume marker, so a fresh session lands on the right bar with no doc to parse. Read that bar's body (`./bin/bit task read <bar> --body`) for its detail, and note the phase it's tagged to (the phase column) so you know what larger capability it's building toward.
...
Move the bar to `doing` (`./bin/bit task update <bar> -s doing`) and roll the track up: if the track isn't already `doing`, set it (`./bin/bit task update <track> -s doing`). Now the board reflects that this step is active.
...
1. **Mark the bar done.** `./bin/bit task update <bar> -s done`. The status field is the resume marker: a fresh session continues at the first bar that isn't `done`, with no doc to parse. (You don't need to tick the checklist boxes inside the bar body — the status field supersedes them.)
2. **Roll the track up.** This is skill logic run through the CLI (the tool doesn't cascade for you):
   - Re-list the bars: `./bin/bit task list --parent <track>`.
   ...
   - Apply both in one call so the track moves once: `./bin/bit task update <track> -d "<edited body>" -s <status>` — pass `-d` only if the phase checkoff changed the body, `-s` only if the status changed. **If neither changed, there's nothing to roll up — skip the call.** (This is the common mid-phase case: finishing a bar when its phase isn't complete yet and the track is already `doing`.)
...
- **Hand-edit `.bit/tasks/*.md`** — every status move and body change goes through `./bin/bit`.
