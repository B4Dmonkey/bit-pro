# Driving bit through `./bin/bit`

The bit_scope, bit_plan, bit_do, and bit_check skills all track their own work inside
`.bit/` by driving the local `./bin/bit` CLI. This is the shared command contract they
...
The single rule that makes the rest safe: **every write goes through `./bin/bit`. Never
...
```bash
./bin/bit task read "$ID" --body | sed 's/- \[ \] Phase 1/- [x] Phase 1/' > body.md
./bin/bit task update "$ID" -d "$(cat body.md)"
```
