...
## Scope
- `launchd/start.go` — new: `Start(ctx context.Context, run Runner, plistPath string) (State, int, error)`
- `launchd/launchd.go` — a `domain()` helper returning `"gui/" + strconv.Itoa(os.Getuid())`, now
...
1. **Write test (RED):** `cmd/start_test.go`
   - [ ] `TestStartCmd_EnablesBeforeBootstrapping`
     ...
     - **Setup:** `t.Setenv("HOME", t.TempDir())`; a fake `launchd.Runner` that appends every call
       ...
       Run `runWithLaunchd(t, lc, startCmdUse)`.
...
2. **Implement (GREEN):**
   - [ ] `launchd/launchd.go`: `func domain() string` returning `"gui/" + strconv.Itoa(os.Getuid())`;
         rewrite `Status`'s `print-disabled` argument to use it.
   - [ ] `launchd/start.go`: `Start(ctx, run, plistPath)` — `run(ctx, "launchctl", "enable", domain()+"/"+Label)`,
   ...
   - [ ] `cmd/start.go`: after the plist write, call `launchd.Start`; print
